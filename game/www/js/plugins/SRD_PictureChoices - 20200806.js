/*:
 * @plugindesc [AddPositionCustom Edition] Allows players to have the ability to select from a collection of pictures.
 * @author SumRndmDde Edit By Nyatama
 *
 * @param Min Scale
 ...
 *
 * @param X Position
 * @desc default:(Graphics.boxWidth - this.width) / 2
 * @default (Graphics.boxWidth - this.width) / 2
 *
 * @param Y Position
 * @desc default:(Graphics.boxHeight - this.height) / 2
 ...
 *
 * 選択肢の位置を変更するプラグインコマンド
 *
 *   SetPictureChoicesPos 100 200
 *
 * 選択肢の位置をプラグインパラメータの値にリセットするプラグインコマンド
 *
 *   SetPictureChoicesPos reset
 *
 *
 * ==========================================================================
 *  End of Help File
...
_.xpos = String(params['X Position'] || (Graphics.boxWidth - this.width) / 2);
_.ypos = String(params['Y Position'] || (Graphics.boxHeight - this.height) / 2);
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
    ...
    if(command.trim().toLowerCase() === 'setpicturechoicespos') {
        if(String(args[0]).trim().toLowerCase() === 'reset') {
			_.xpos = String(params['X Position'] || (Graphics.boxWidth - this.width) / 2);
            _.ypos = String(params['Y Position'] || (Graphics.boxHeight - this.height) / 2);
		} else {
            _.xpos = String(args[0]);
            _.ypos = String(args[1]);
        }
    }
};
...
*/
		sprite.x = (rect.x/1.1) + (rect.width/2);
		sprite.y = (rect.y/1.3) + (rect.height/1);
